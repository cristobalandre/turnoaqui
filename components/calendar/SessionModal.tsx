    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-md bg-black/70 animate-in fade-in duration-300" onClick={onClose}>
      <div className="bg-[#0c0c0e] border border-zinc-800 w-full max-w-lg rounded-[40px] p-10 shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
           <button onClick={onStartSession} className="py-5 bg-zinc-900 border border-zinc-800 text-white rounded-3xl text-[9px] font-black tracking-[0.2em] hover:bg-emerald-500/10 hover:border-emerald-500/30 transition-all flex flex-col items-center gap-2">
             <span className="text-xl">▶</span> INICIAR SESIÓN
           </button>
           <button onClick={onStopSession} className="py-5 bg-zinc-900 border border-zinc-800 text-white rounded-3xl text-[9px] font-black tracking-[0.2em] hover:bg-red-500/10 hover:border-red-500/30 transition-all flex flex-col items-center gap-2">
        ...
        <div className="flex gap-4">
          <button onClick={onDelete} className="p-5 bg-red-500/10 text-red-500 rounded-3xl hover:bg-red-500/20 transition-all">🗑️</button>
          <button onClick={onSave} className="flex-1 py-6 bg-white text-black text-[11px] font-black tracking-[0.3em] rounded-3xl hover:bg-emerald-400 transition-all uppercase shadow-2xl">Guardar Cambios</button>
        </div>
      </div>
    </div>
  );
